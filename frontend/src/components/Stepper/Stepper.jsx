...
export default function Stepper({
  ...
  useEffect(() => {
    if (
      currentStepOverride !== undefined &&
      currentStepOverride !== null &&
      currentStepOverride !== currentStep
    ) {
      setDirection(currentStepOverride > currentStep ? 1 : -1);
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators = false }) {
  ...
  return (
    ...
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: "hsl(var(--surface-container))", color: "hsl(var(--on-surface-variant))" },
