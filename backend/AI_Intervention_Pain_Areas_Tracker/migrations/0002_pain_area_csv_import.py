...
SQL_DROP = r"""
DROP FUNCTION IF EXISTS process_pain_area_csv_import(uuid);
DROP TABLE IF EXISTS ai_pain_area_csv_staging;
"""


def create_pipeline(apps, schema_editor):
...
def drop_pipeline(apps, schema_editor):
    if schema_editor.connection.vendor != "postgresql":
        return
    with schema_editor.connection.cursor() as cursor:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("AI_Intervention_Pain_Areas_Tracker", "0001_initial"),
    ]

    operations = [
        migrations.RunPython(create_pipeline, drop_pipeline),
    ]
