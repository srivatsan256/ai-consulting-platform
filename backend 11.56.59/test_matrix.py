...
class MatrixTester:
    ...
    def test_auth_extras(self):
        # switch company (matrix admin has a membership in company 1)
        r = self.post(
            f"{BASE}/auth/switch/",
            json={"company_id": 1},
        )
    ...
    def crud(self, module, base, post_payload=None, schema_ref=None):
        ...
        for name in USER_FK_FIELDS:
            if name in post_payload:
                post_payload[name] = self.user_id
        if "participants" in post_payload:
            post_payload["participants"] = [self.user_id]
        if "company" in post_payload:
            post_payload["company"] = self.company_id
        if "parent" in post_payload:
            post_payload["parent"] = None
        ...
        if detail_ep:
            ...
            patch_payload = {}
            for k in list(post_payload.keys())[:1]:
                if isinstance(post_payload[k], str):
                    patch_payload[k] = post_payload[k] + "x"
                    break
            if patch_payload:
    ...
    def run(self):
        ...
        self.crud("Company Members", "/memberships")
        self.crud("Roles", "/roles")
        self.crud("Permissions", "/permissions")

        # --- Departments / Teams / Projects ---
        self.crud("Departments", "/departments")
        self.crud("Teams", "/teams")
        ...
        self.crud("Discovery", "/discovery")
        self.crud("Documents", "/documents")
        self.crud("Document Templates", "/document-templates")

        # --- AI Engine ---
        self.crud("AI Engine", "/ai-engine/assessments")
        self.crud("AI Engine", "/ai-engine/use-cases")
        self.test_get("AI Engine", "/ai-engine/prompts")
        self.test_get("AI Engine", "/ai-engine/prompts/matrix_template")

        # --- Knowledge Base / Chat ---
        self.crud("Knowledge Base", "/knowledge-base")
        self.crud("Chat", "/chat/conversations")
        self.crud("Chat", "/chat/messages")

        # --- Reviews / Approvals / Workflows ---
        self.crud("Reviews", "/reviews")
        self.crud("Approvals", "/approvals")
        self.crud("Workflows", "/workflows")
        self.test_get("Workflows", "/workflows/executions")
